...
static volatile VAR(boolean, ECUM_VAR_CLEARED_BOOLEAN) EcuM_Prv_flgSyncWakeupDetected_b;
...
volatile VAR( boolean, ECUM_VAR_CLEARED_BOOLEAN ) EcuM_Prv_PollSlaveCoreContinue_b;
...
static FUNC(void, ECUM_CODE) EcuM_Prv_PollingSequence(VAR(uint16, AUTOMATIC) dataSleepModeHandle_u16);
...
static FUNC(void, ECUM_CODE) EcuM_Prv_HaltingSequence(VAR(uint16, AUTOMATIC) dataSleepModeHandle_u16)
    ...
    VAR( EcuM_WakeupSourceType, AUTOMATIC ) dataPendingWakeupEvents_u32 = ECUM_NO_WKUP_SRC;
    /*Internal variable which holds the bit mask of wakeup events that are in validated state*/
    VAR( EcuM_WakeupSourceType, AUTOMATIC ) dataValidatedWakeupEvents_u32 = ECUM_NO_WKUP_SRC;
...
#if(ECUM_CFG_MULTICORE_ENABLED  ==  STD_ON)
 VAR(uint16, AUTOMATIC) Core_ID;
#endif//ECUM_CFG_MULTICORE_ENABLED

    VAR( EcuM_Prv_OperationType_Enum_ten, AUTOMATIC )  flgOperation_en = ECUM_PRV_POLL_SEQUENCE_E;
